...
Rewrite notes (from the prior version):
    ...
    - Restructured into three views per the product spec: OPERATE
      (default -- perceive/decide/act loop), MISSION (objective-driven autonomous planning, run/replay, connectivity,
      reset), PROVE (collapsible engineering evidence). No ML/runtime/benchmark methodology was touched.
...
from ecosphere_ui_adapter import to_ui_state
...
MISSION = {
    "objective": "Complete an environmental survey while responding locally to hazards.",
    "priority": "Safety-first coverage",
    "strategy": "Monitor → respond → recover",
    "success_text": "Mission ends SAFE after the hazard response and recovery step.",
    "success": "Final state must be SAFE after the recovery step.",
    "steps": [
...
st.set_page_config(
    page_title="ECOsphere — V3 Mission",
    page_icon="◆",
    layout="wide",
...
def mission_success(mission, trace):
    states = [item["state"] for item in trace]
    return bool(states) and states[-1] == "SAFE"
...
def execute_current_input():
    ...
    if is_new_reading:
        ...
        ui_snapshot = to_ui_state(result, cloud_connected=st.session_state.cloud_online)
        st.session_state.mission_history.append({
            ...
            "presentation_state": ui_snapshot.presentation_state,
            "decision": ui_snapshot.decision,
            "rover_motion": ui_snapshot.rover_motion,
            "route": ui_snapshot.route,
...
with tab_operate:
    ...
    mission_status = (
        "AUTONOMOUS SURVEY · READY"
        if not st.session_state.autonomous_complete
        else "AUTONOMOUS SURVEY · LAST RUN COMPLETE"
    )
    st.markdown(
        f'<div class="mission-strip"><b>{mission_status}</b>'
        f'<span>{MISSION["strategy"]}</span></div>',
        unsafe_allow_html=True,
    )
    ...
    ui = to_ui_state(result, cloud_connected=st.session_state.cloud_online)
    state, action, state_class = state_meta(result["hazard_state"])
    confidence = confidence_of(result)
    summary = runtime.summary()

    # V3 mission communication layer: values are read from the adapter;
    # no new risk/decision logic is performed here.
    st.markdown(
        f"""<div class="mission-strip"><b>MISSION 01 · {ui.presentation_state.replace('_',' ')}</b>
        <span>ROVER · {ui.rover_motion} · {ui.rover_speed} &nbsp;|&nbsp; ROUTE · {ui.route}</span></div>""",
        unsafe_allow_html=True,
    )

    v1, v2, v3, v4 = st.columns([1.25, 1.0, 1.0, 1.25])
    with v1:
        st.metric("ENVIRONMENT", ui.hazard_state)
    with v2:
        st.metric("RISK", f"{ui.risk:.3f}")
    with v3:
        st.metric("TREND", ui.trend.replace('_',' '))
    with v4:
        st.metric("ACTION", ui.decision.replace('_',' '))

    why_cols = st.columns([1.0, 1.0, 2.0])
    with why_cols[0]:
        st.caption(f"Gas {ui.gas:.0f} ppm")
    with why_cols[1]:
        st.caption(f"Tilt {ui.tilt:.1f}° · Vibration {ui.vibration:.2f}g")
    with why_cols[2]:
        with st.expander("WHY DID THE ROVER DO THAT?", expanded=False):
            st.markdown(f"**{ui.decision.replace('_',' ')}**")
            st.write(ui.explanation)
            st.write(ui.next_step)
            st.caption(f"Trigger: {ui.trigger} · Priority: {ui.priority}")

    if st.session_state.mission_history:
        timeline_items = []
        for item in st.session_state.mission_history[-8:]:
            label = item.get("presentation_state", item.get("state", "UNKNOWN")).replace("_", " ")
            timeline_items.append(f'<span class="timeline-node active">{label}</span>')
        st.markdown(
            '<div class="timeline"><div class="card-title">Mission timeline · observed</div>'
            '<div class="timeline-row">' +
            ' <span class="timeline-arrow">→</span> '.join(timeline_items) +
            '</div></div>',
            unsafe_allow_html=True,
        )

    route_visual = "REROUTING" if ui.route == "REROUTING" else "PRIMARY"
    rover_visual = (
        "STOPPED" if ui.rover_motion == "STOPPED"
        else ("SLOWING" if ui.rover_speed == "REDUCED" else "MOVING")
    )
    hazard_mark = "●" if ui.hazard_state != "SAFE" else ""
    st.markdown(
        f"""<div class=\"hero-panel\" style=\"margin:.55rem 0 .7rem;\">
        <div class=\"kicker\">MISSION WORLD · LIVE PHYSICAL STATE</div>
        <div style=\"display:flex;align-items:center;justify-content:space-between;gap:1rem;min-height:96px;\">
          <div style=\"font-size:2.2rem;\">🛻</div>
          <div style=\"flex:1;border-top:2px solid rgba(148,163,184,.35);position:relative;\">
            <span style=\"position:absolute;left:42%;top:-.72rem;font-size:.7rem;opacity:.6;\">{hazard_mark} HAZARD</span>
          </div>
          <div style=\"font-size:.72rem;font-weight:750;letter-spacing:.08em;\">{rover_visual}<br><span style=\"opacity:.5;\">{route_visual}</span></div>
        </div>
        <div style=\"font-size:.72rem;opacity:.58;\">Action: {ui.rover_action.replace('_',' ')} · Decision path: {ui.decision_path}</div>
        </div>""",
        unsafe_allow_html=True,
    )

    if ui.verification is not None:
        verification_name = str(ui.verification.get("verification", "UNKNOWN")).replace('_',' ')
        st.info(f"CHECKING RESPONSE · {verification_name}")

    if ui.recovery:
        recovery_state = str(ui.recovery.get("state", "UNKNOWN")).replace('_',' ')
        falling = ui.recovery.get("falling_count", 0)
        required = ui.recovery.get("required_observations", 2)
        st.caption(f"Recovery: {recovery_state} · {falling}/{required} confirming observations")
...
with tab_mission:
    ...
    st.markdown(
        f"""<div class="mission-objective">
        <b>AUTONOMOUS ENVIRONMENTAL SURVEY</b><br>
        {MISSION["objective"]}<br><br>
        <span class="mono-small">
        PRIORITY · {MISSION["priority"]}<br>
        STRATEGY · {MISSION["strategy"]}<br>
    ...
    st.caption(
        "This is the autonomous mission layer: it defines the mission sequence and success condition. "
        "It does not alter the trained hazard detector."
    )

    pcol1, pcol2, pcol3 = st.columns([1.1, .8, 1.0])
    with pcol1:
        st.select_slider(
            "Autonomous mission pace",
            options=["Slow", "Normal", "Fast"],
            key="pace",
            help="Controls demonstration pacing only; it does not alter the edge decision.",
        )
    with pcol2:
        st.metric("Mission states", len(MISSION["steps"]))
    with pcol3:
        st.metric("Strategy", MISSION["strategy"].split(" → ")[0])
    ...
    if st.button("▶ Start autonomous mission", type="primary", use_container_width=True):
        ...
        st.session_state.autonomous_trace = []
        st.session_state.autonomous_pace = st.session_state.pace
        st.session_state.autonomous_complete = False
        ...
        for idx, (step_label, values) in enumerate(steps):
            g, t, v = values
            events_before = len(runtime.event_log)
            ...
            with mission_rover_box.container():
                render_rover_panel(live_result["hazard_state"], is_new_reading=True)
            ...
            st.session_state.autonomous_trace.append(trace_item)
            st.session_state.mission_history.append({
                "label": step_label, "gas": g, "tilt": t, "vibration": v,
                "state": live_state, "cache": live_result["cache_hit"],
                "rf": live_result["rf_executed"],
            })

            cache_text = "CACHE HIT" if live_result["cache_hit"] else "RF EXECUTED"
            mission_box.markdown(
                f'<div class="panel"><div class="kicker">AUTONOMOUS STEP {idx+1}/{len(steps)}</div>'
            ...
            progress.progress((idx + 1) / len(steps), text=f"{step_label} · {live_state}")
            _time.sleep(PACE_DELAY[st.session_state.pace])

        st.session_state.pending_sensor_values = steps[-1][1]
        st.session_state.last_input = None
        st.session_state.autonomous_complete = True

        states = [x["state"] for x in st.session_state.autonomous_trace]
        success = mission_success(MISSION, st.session_state.autonomous_trace)

        st.markdown(
            f"""<div class="outcome">
<b>AUTONOMOUS MISSION · {"SUCCESS" if success else "OBJECTIVE NOT MET"}</b><br>
<span class="mono-small">{MISSION["success"]}</span><br><br>
{len(states)} states observed · {states.count("CRITICAL")} critical ·
{states.count("ELEVATED")} elevated ·
{sum(1 for x in st.session_state.autonomous_trace if x["cache"])} cache-assisted decisions
</div>""",
            unsafe_allow_html=True,
        )

    if st.session_state.autonomous_complete and st.session_state.autonomous_trace:
        st.markdown("##### REPLAY LAST AUTONOMOUS MISSION")
        st.caption(
            f'Recorded run · Autonomous Environmental Survey · '
            f'{len(st.session_state.autonomous_trace)} validated decisions · '
            f'original pace {st.session_state.autonomous_pace}. '
            'Replay is visual only: no model inference, cloud analysis, events, or mission-history writes.'
        )
        if st.button("↻ Replay autonomous run", use_container_width=True):
            trace = st.session_state.autonomous_trace
            saved_rover = {
                "position": st.session_state.get("rover_position", 0.0),
                "reroutes": list(st.session_state.get("rover_reroute_points", [])),
                "stopped": st.session_state.get("rover_stopped", False),
            }

            reset_rover()
            replay_cols = st.columns([1.4, 1])
            with replay_cols[0]:
                replay_rover_box = st.empty()
            with replay_cols[1]:
                replay_box = st.empty()

            replay_delay = PACE_DELAY[st.session_state.autonomous_pace]
            for item in trace:
                with replay_rover_box.container():
                    render_rover_panel(item["state"], is_new_reading=True)
                replay_box.markdown(
                    f"""<div class="panel">
                    <div class="kicker">REPLAY · STEP {item["step"]}/{len(trace)}</div>
                    <div class="big-state">{item["state"]}</div>
                    <b>{item["label"]}</b><br>
                    Gas {item["gas"]:.0f} ppm · Tilt {item["tilt"]:.1f}° · Vibration {item["vibration"]:.2f} g<br>
                    <b>{item["action"]}</b> · {"CACHE HIT" if item["cache"] else "RF EXECUTED"}
                    </div>""",
                    unsafe_allow_html=True,
                )
                _time.sleep(replay_delay)

            # Replay must not alter the live mission state.
            st.session_state.rover_position = saved_rover["position"]
            st.session_state.rover_reroute_points = saved_rover["reroutes"]
            st.session_state.rover_stopped = saved_rover["stopped"]
    ...
    with mcol1:
        ...
        if st.session_state.cloud_online:
            st.markdown(
                ...
                '<span class="mono-small">New edge events may request configuration adaptation.</span></div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                '<div class="signal-danger"><b>CLOUD OFFLINE</b><br>'
                '<span class="mono-small">Local inference continues; last edge configuration is retained.</span></div>',
                unsafe_allow_html=True,
            )
    ...
    with mcol2:
        ...
        st.caption("Reset clears the active runtime, rover, mission record, autonomous trace, configuration, and inputs.")
        st.button("⟲ Reset Mission", use_container_width=True, on_click=reset_mission)

        if st.session_state.mission_history:
            st.markdown("##### LATEST AUTONOMOUS RECORD")
            for item in st.session_state.mission_history[-5:]:
            ...
            st.caption("No autonomous mission recorded yet.")
