...
Rewrite notes (from the prior version):
    ...
    - The "Replay last autonomous mission" block had its body
      (replay_cols/replay_rover_box/replay_box/the for-loop) dedented to
      module level, outside the `if st.button(...)` check and even outside
      the `if autonomous_complete...` check. That meant:
        1) `trace` was undefined on any rerun before a replay button click
           in this session, raising NameError as soon as the loop was hit.
        2) once `trace` existed, the replay loop (including its
           time.sleep() pacing) re-ran on *every* rerun of the whole app --
           any slider touch, any tab switch -- not just on the button
           click, making the console block/animate unexpectedly.
      Fixed by nesting the entire replay block back inside
      `if st.button("↻ Replay autonomous run", ...)`, so it only executes
      once, on click, and `trace` is always defined before use. No other
      logic, ML/runtime/benchmark methodology, or evidence numbers were
      touched.
...
MISSION = {
    ...
    ("Recovery — hazard clear", SCENARIOS["Normal"]),
    ("Reroute — bypass 1", SCENARIOS["Normal"]),
    ("Reroute — bypass 2", SCENARIOS["Normal"]),
    ("Reroute — bypass 3", SCENARIOS["Normal"]),
    ("Reroute — bypass 4", SCENARIOS["Normal"]),
    ("Reroute — bypass 5", SCENARIOS["Normal"]),
    ("Reroute — bypass 6", SCENARIOS["Normal"]),
],
}
    ...
    if st.button("▶ Start autonomous mission", type="primary", use_container_width=True):
        ...
        for idx, (step_label, values) in enumerate(steps):
            ...
            trace_item = {
                ...
                "vibration": v,
                "state": live_state,
                "action": f"{live_action} → ROVER {rover_state}",
...
{sum(1 for x in st.session_state.autonomous_trace if x["cache"])} cache-assisted decisions
    ...
    if st.session_state.autonomous_complete and st.session_state.autonomous_trace:
        ...
        if st.button("↻ Replay autonomous run", use_container_width=True):
            ...
            for item in trace:
                # Replay is READ-ONLY.
                # It displays the recorded rover response but never calls
                # render_rover_panel() and never modifies live rover state.

                with replay_rover_box.container():
                    st.markdown(
                        f"""
                        <div class="panel">
                            <div class="kicker">RECORDED ROVER RESPONSE</div>
                            <div class="big-state">{item["action"].split(" → ROVER ")[-1]}</div>
                            <div class="mono-small">
                                Step {item["step"]}/{len(trace)}
                            </div>
                        </div>
                        """,
                        unsafe_allow_html=True,
                ...
                replay_box.markdown(
                    ...
                    <div class="panel">
                        ...
                        Gas {item["gas"]:.0f} ppm ·
                        Tilt {item["tilt"]:.1f}° ·
                        Vibration {item["vibration"]:.2f} g<br>
                        <b>{item["action"]}</b> ·
                        {"CACHE HIT" if item["cache"] else "RF EXECUTED"}
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
